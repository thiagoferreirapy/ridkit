...
type Admin={id:number;name:string;email:string;role:string};type State={admin:Admin|null;loading:boolean;login:(email:string,password:string,remember?:boolean)=>Promise<void>;logout:()=>Promise<void>;adminFetch:(url:string,init?:RequestInit)=>Promise<Response>};
...
export function AdminAuthProvider({children}:{children:React.ReactNode}){const router=useRouter(),[admin,setAdmin]=useState<Admin|null>(null),[loading,setLoading]=useState(true);
  ...
  const login=async(email:string,password:string,remember=false)=>{const response=await request("/api/admin-auth/login",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email,password,remember})}),body=await response.json();if(!response.ok)throw new Error(body.error);setAdmin(body.data.user);router.replace("/admin");router.refresh();};
