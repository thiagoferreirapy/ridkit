...
type PaymentOrder={id:number;customer_id:number;order_number:string;payment_method:string;payment_status:string;total_cents:number;shipping_address_json:string;provider_payment_id:string|null;pix_qr_code:string|null;pix_qr_image:string|null;pix_expires_at:string|null};
...
export async function POST(request:Request){
  try{
    ...
    if(!customer?.cpf)throw new Error("BAD_REQUEST:Cadastre um CPF válido antes de pagar");
    const address=JSON.parse(order.shipping_address_json) as Address;
    let asaasCustomerId=customer.asaas_customer_id;
    if(!asaasCustomerId){const remote=await createAsaasCustomer({name:customer.name,cpf:customer.cpf,email:customer.email,phone:customer.phone,externalReference:`ridekit-customer-${customer.id}`,address});asaasCustomerId=remote.id;db.prepare("UPDATE customers SET asaas_customer_id=? WHERE id=? AND asaas_customer_id IS NULL").run(asaasCustomerId,customer.id);}
