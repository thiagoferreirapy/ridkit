...
export async function POST(request: Request) {
  ...
  try {
    ...
    const result=db.prepare(`INSERT INTO products(brand_id,category_id,name,slug,sku,description,price_cents,compare_at_cents,cost_cents,color,finish,shell_material,weight_grams,solar_visor,pinlock_ready,featured,is_new,launch_starts_at,launch_ends_at,offer_starts_at,offer_ends_at,active) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(value.brand_id,value.category_id,value.name,productSlug,value.sku,value.description,value.price_cents,value.compare_at_cents??null,value.cost_cents,value.color,value.finish??null,value.shell_material??null,value.weight_grams??null,value.solar_visor,value.pinlock_ready,value.featured,value.is_new,value.launch_starts_at??null,value.launch_ends_at??null,value.offer_starts_at??null,value.offer_ends_at??null,value.active);
