...
export async function GET(_request:Request,{params}:{params:Promise<{id:string}>}) { try {const {id}=await params; const db=getDb(); const order=db.prepare(`SELECT o.*,c.name AS customer,c.email,c.phone FROM orders o JOIN customers c ON c.id=o.customer_id WHERE ${/^\d+$/.test(id)?"o.id":"o.order_number"}=?`).get(/^\d+$/.test(id)?Number(id):id) as Record<string,unknown>|undefined; if(!order)throw new Error("NOT_FOUND"); const orderId=Number(order.id); return ok({...order,shipping_address:JSON.parse(String(order.shipping_address_json)),items:db.prepare("SELECT oi.*,pi.url AS image_url FROM order_items oi LEFT JOIN product_images pi ON pi.product_id=oi.product_id AND pi.position=0 WHERE oi.order_id=?").all(orderId),events:db.prepare("SELECT * FROM order_events WHERE order_id=? ORDER BY occurred_at").all(orderId)});}catch(error){return apiError(error);} }
...
export async function PATCH(request:Request,{params}:{params:Promise<{id:string}>}) {
  ...
  try {
    ...
    const order=db.prepare("SELECT status FROM orders WHERE id=?").get(orderId) as {status:string}|undefined; if(!order)throw new Error("NOT_FOUND");
    ...
    db.exec("COMMIT"); return ok({id:orderId,status:value.status});
