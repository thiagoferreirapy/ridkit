...
export async function POST(request:Request) {
  const db=getDb();
  try { const settings=getStoreSettings();if(settings.purchase_mode!=="site")throw new Error("BAD_REQUEST:As compras estão sendo finalizadas pelo WhatsApp");const user=await requireUser(); const value=orderSchema.parse(await request.json());
    ...
    if(value.payment_method==="pix"&&settings.pix_discount_percent>0)discount+=Math.round((subtotal-discount)*settings.pix_discount_percent/100);
