...
export async function POST(request:Request) {
  const db=getDb();
  try { const value=orderSchema.parse(await request.json()); db.exec("BEGIN IMMEDIATE");
    const customer=db.prepare("SELECT id FROM customers WHERE id=?").get(value.customer_id); if(!customer)throw new Error("BAD_REQUEST:Cliente inválido");
    ...
    const shipping=value.shipping_method==="Expressa"?2990:subtotal>=29900||value.shipping_method==="Retirada"?0:1990; const total=subtotal-discount+shipping; const last=(db.prepare("SELECT COALESCE(MAX(id),0)+1 AS next FROM orders").get() as {next:number}).next; const orderNumber=`${10234+last}`;
    const orderResult=db.prepare(`INSERT INTO orders(customer_id,coupon_id,order_number,status,payment_method,payment_status,subtotal_cents,discount_cents,shipping_cents,total_cents,shipping_method,shipping_address_json) VALUES(?,?,?,'pending',?,'pending',?,?,?,?,?,?)`).run(value.customer_id,couponId,orderNumber,value.payment_method,subtotal,discount,shipping,total,value.shipping_method,JSON.stringify(value.shipping_address)); const orderId=Number(orderResult.lastInsertRowid);
    ...
    db.prepare("INSERT INTO order_events(order_id,status,title,description) VALUES(?,'pending','Pedido recebido','Aguardando confirmação do pagamento.')").run(orderId); if(couponId)db.prepare("UPDATE coupons SET used_count=used_count+1 WHERE id=?").run(couponId); db.exec("COMMIT"); return created({id:orderId,order_number:orderNumber,status:"pending",subtotal_cents:subtotal,discount_cents:discount,shipping_cents:shipping,total_cents:total});
