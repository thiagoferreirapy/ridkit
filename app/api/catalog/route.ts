...
export function GET() { const db=getDb(); return ok({
  ...
  categories:db.prepare("SELECT c.*,COUNT(DISTINCT p.id) AS product_count,MAX(CASE WHEN v.size IS NOT NULL AND v.size<>'Único' THEN 1 ELSE 0 END) AS has_sizes FROM categories c LEFT JOIN products p ON p.category_id=c.id AND p.active=1 LEFT JOIN product_variants v ON v.product_id=p.id AND v.active=1 WHERE c.active=1 GROUP BY c.id ORDER BY c.name").all(),
  ...
  filter_products:db.prepare(`SELECT p.id,p.price_cents,p.featured,c.slug AS category_slug,b.slug AS brand_slug,
    (SELECT GROUP_CONCAT(size,'|') FROM (SELECT DISTINCT pv.size AS size FROM product_variants pv WHERE pv.product_id=p.id AND pv.active=1 AND pv.size<>'Único')) AS sizes_csv
